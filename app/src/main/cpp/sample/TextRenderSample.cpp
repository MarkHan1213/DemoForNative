...
void TextRenderSample::Draw(int screenW, int screenH) {
    ...
    RenderText(BYTE_FLOW, sizeof(BYTE_FLOW) / sizeof(BYTE_FLOW[0]) - 1, -0.9f, -0.2f, 1.0f,
               glm::vec3(0.7, 0.4f, 0.2f), viewport);
...
void TextRenderSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    ...
    //转化为弧度角
    float radiansX = static_cast<float>(SAMPLE_MATH_PI / 180.0f * angleX);
    float radiansY = static_cast<float>(SAMPLE_MATH_PI / 180.0f * angleY);
